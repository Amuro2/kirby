...
public class CutterBlockScript : PeriodicDecisionAI {
        ...
        if(rangeDetect.hasObjects()) {
            ...
            if(charging) {
                ...
                if(attackTimeout <= 0) {
                    ...
                    GameObject projectile = Instantiate(Resources.Load<GameObject>("collision_boxes/TotemCutter"));
                    
                    projectile.GetComponent<Hitbox>().tagToHit = "Player";
                    
                    float sign = Mathf.Sign(target.transform.position.x - transform.position.x);
                    
                    projectile.GetComponent<Rigidbody2D>().velocity = new Vector2(sign * 20, 0);
                    
                    Vector3 position = transform.position;
                    
                    position.x += sign * transform.localScale.x/2;
